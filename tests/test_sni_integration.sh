...
cleanup() {
    ...
    [[ -z "$tls_pid_two" ]] || kill "$tls_pid_two" 2>/dev/null || true
    rm -rf -- "$test_dir"
}
trap cleanup EXIT
...
openssl req -x509 -newkey rsa:2048 -nodes -days 1 -subj '/CN=backend-one' \
    -keyout "$test_dir/key.pem" -out "$test_dir/cert.pem" >/dev/null 2>&1
openssl req -x509 -newkey rsa:2048 -nodes -days 1 -subj '/CN=backend-two' \
    -keyout "$test_dir/key-two.pem" -out "$test_dir/cert-two.pem" >/dev/null 2>&1
openssl s_server -quiet -accept 127.0.0.1:19443 \
    -key "$test_dir/key.pem" -cert "$test_dir/cert.pem" >"$test_dir/tls.log" 2>&1 &
tls_pid=$!
openssl s_server -quiet -accept 127.0.0.1:19444 \
    -key "$test_dir/key-two.pem" -cert "$test_dir/cert-two.pem" >"$test_dir/tls-two.log" 2>&1 &
tls_pid_two=$!

export GOST_SNI_ALLOW_NON_ROOT=1 GOST_SNI_SKIP_SYSTEMD=1 NO_COLOR=1
export GOST_SNI_STATE_DIR="$test_dir/state"
export GOST_SNI_CORE_BIN="$test_dir/gost"
manager="$repo_dir/manager/gost-sni"
"$manager" init
"$manager" domain add example.com
"$manager" forward add 18443 127.0.0.1 19443 integration-one
"$manager" forward add 18444 127.0.0.1 19444 integration-two

config="$test_dir/state/gost.yml"
grep -Fq 'addr: "0.0.0.0:18443"' "$config"
grep -Fq 'addr: "127.0.0.1:19443"' "$config"
grep -Fq 'addr: "0.0.0.0:18444"' "$config"
grep -Fq 'addr: "127.0.0.1:19444"' "$config"
if grep -Eiq 'type:[[:space:]]*udp' "$config"; then
    printf 'UDP service unexpectedly present in generated config\n' >&2
    exit 1
fi
...
for _ in {1..30}; do
    if (echo >/dev/tcp/127.0.0.1/18443) 2>/dev/null && \
        (echo >/dev/tcp/127.0.0.1/18444) 2>/dev/null; then break; fi
    sleep 0.1
done
kill -0 "$gost_pid"
...
timeout 5 openssl s_client -brief -connect 127.0.0.1:18444 -servername example.com \
    </dev/null >/dev/null 2>&1
tls_rejected notexample.com
tls_rejected example.net

subject_one="$(timeout 5 openssl s_client -connect 127.0.0.1:18443 -servername example.com \
    </dev/null 2>/dev/null | openssl x509 -noout -subject)"
subject_two="$(timeout 5 openssl s_client -connect 127.0.0.1:18444 -servername example.com \
    </dev/null 2>/dev/null | openssl x509 -noout -subject)"
[[ "$subject_one" == *backend-one* ]]
[[ "$subject_two" == *backend-two* ]]
